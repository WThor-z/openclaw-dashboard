...
面向公众发布的版本文章与技术博客放在 `news/`。

## 功能亮点

- Agent Workspace 主入口为 `/dashboard`
- V2 Agent Runtime 入口为 `/agents/:agentId/runtime`，支持 conversations、schedules、heartbeat 与 memory
- 本地 token 鉴权登录流程
- Agent 卡片与状态轮询
- 600px 宽侧边抽屉查看工作区
- 递归文件树浏览 Agent 工作区
...
- `news` - 面向公众的版本文章与发布技术博客
- `tests/e2e` - Playwright 浏览器回归测试
- `tests/contracts` - 跨应用契约测试
- `tools/simulator` - 模拟器实现与启动脚本包
- `tools` - 本地运维脚本、可靠性辅助模块与工作区工具
- `tests/verification` - 环境、安全与运维校验入口
...
- 中文导航：`docs/directory-map.zh-CN.md`
- English map: `docs/directory-map.md`
- 版本文章：`news/`

这些文档共同构成“目录该怎么放、版本文章去哪里看”的统一入口。
...
dashboard 现已包含 V2 Agent Runtime，支持通过持久化 conversations、scheduled jobs、heartbeat 配置与 memory bindings 与 Agent 交互。

Runtime 路由：

- `/agents/:agentId/runtime` - runtime 主界面，包含 Conversations、Schedules、Heartbeat、Memory 标签页
- `/agents/:agentId/runtime/conversations/:conversationId` - 特定对话线程

Runtime 能力：

- **Conversations** - 创建、列表展示、发送消息，支持持久化历史记录
- **Schedules** - 基于 cron 的定期提示，附带执行历史
- **Heartbeat** - Agent 定期心跳检查配置
- **Memory** - 分作用域的 memory binding（conversation、agent、system），仅支持 secretRef 凭据引用
...
- Playwright 也覆盖 V2 runtime 流程，包括 conversations、schedules、heartbeat 与 memory
