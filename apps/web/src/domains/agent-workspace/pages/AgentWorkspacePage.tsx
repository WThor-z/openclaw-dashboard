...
import { AgentList } from "../../../shared/components/AgentList.js";
...
import {
  ...
  type DaemonWorkspaceNode,
  sortPreviewPaths
} from "../lib/markdown.js";
import { loadSelectedAgentId, loadStoredPinnedNotes, saveSelectedAgentId } from "../lib/storage.js";
  ...
  useEffect(() => {
    ...
    const loadPreviewFiles = async () => {
      ...
      try {
        ...
        setSelectedPreviewPath((currentPath) =>
          currentPath && nextPaths.includes(currentPath) ? currentPath : (nextPaths[0] ?? null)
        );
      } catch {
        if (!cancelled) {
          setPreviewPaths([]);
              ...
              <div data-testid="drawer-placeholder" className="grid gap-4 md:grid-cols-2">
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <p className="text-[10px] font-bold uppercase tracking-[0.35em] text-[#1f5ba6]">
                    {t("workspace.overview.card.overview")}
                  </p>
                  <p
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <p className="mt-2 text-xs text-slate-600">
                    {t("workspace.overview.openPreviewHint")}
                  </p>
                </div>
              </div>
              ...
              {agents.length > 0 ? (
                <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-2 text-sm text-slate-600">
                        Open the runtime shell for a specific agent without replacing workspace
                        routes.
                      </p>
                    </div>
                  </div>
                  ...
                  <ul className="mt-4 grid gap-2 md:grid-cols-2">
                    {agents.map((agent) => (
                      <li key={`runtime-${agent.id}`}>
                        <Link
                          data-testid={`agent-runtime-link-${agent.id}`}
                          to={`/agents/${encodeURIComponent(agent.id)}/runtime`}
                          className="flex items-center justify-between rounded-lg border border-transparent bg-transparent px-3 py-2 text-[13px] font-semibold text-slate-700 transition-all duration-200 hover:translate-x-[1px] hover:border-[#1f5ba6]/25 hover:bg-[#eef5ff] hover:text-[#123f77]"
                        >
            ...
            >
              ...
              <div className="flex flex-1 flex-col overflow-hidden p-6">
                {isDrawerLoading ? (
                  ...
                  <EmptyState
                    title={t("workspace.preview.unavailable")}
                    message={previewError}
                    className="h-full"
                  />
                ) : previewPaths.length === 0 ? (
                  <EmptyState
                    title={t("workspace.preview.none")}
                    message={t("workspace.preview.noneHint")}
                    className="h-full"
                  />
                ) : (
                  <>
                    <ul
                      ...
                      aria-label={t("workspace.pinned.title")}
                    >
                          ...
                          <div
                            ...
                            className={`flex cursor-pointer items-center justify-between px-4 py-2 text-left text-xs transition-colors ${
                              selectedPreviewPath === path
                                ? "bg-[#ecf3ff] text-[#123f77]"
                                : "text-slate-600 hover:bg-slate-100 hover:text-slate-900"
                            }`}
                          >
                            <span className="truncate">{path}</span>
                            {selectedPreviewPath === path ? (
                              <span className="ml-3 text-[10px] uppercase tracking-[0.24em] text-[#1f5ba6]">
                        ...
                        <div className="space-y-3">
                          <div>
                            ...
                            <p className="text-[10px] text-slate-500">
                              {previewModifiedAt
                                ? t("workspace.preview.modifiedPrefix", {
                                    value: previewModifiedAt
                                  })
                                : t("workspace.preview.modifiedUnknown")}
                            </p>
                          </div>
